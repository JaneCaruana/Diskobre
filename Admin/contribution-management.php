...
$ownerNum = mysqli_query($link, "SELECT COUNT(*) AS allEstablishOwner FROM establishment  JOIN user ON establishment.user_fk = user.user_id where user.user_type = '2';");//? CALL QUERY for all establishment
...
$pendingContr = mysqli_query($link, "SELECT COUNT(*) AS totalPending FROM establishment  JOIN user ON establishment.user_fk = user.user_id where user.user_type = '2' AND status = 1" );//? CALL QUERY for all establishment
...
$approvedContr = mysqli_query($link, "SELECT COUNT(*) AS totalApprove FROM establishment JOIN user ON establishment.user_fk = user.user_id where user.user_type = '2' AND   status = 2");//? CALL QUERY for all establishment
$getApprove=mysqli_fetch_array($approvedContr);

$totalApprv = $getApprove['totalApprove'];

// ? Number of disapproved contribution
$disapprovedContr = mysqli_query($link, "SELECT COUNT(*) AS totalDisapprove FROM establishment JOIN user ON establishment.user_fk = user.user_id where user.user_type = '2' AND   status = 3");//? CALL QUERY for all establishment
...
<body>
  <div class="body-wrapper">
    <div class="left-panel"><img src="images/admin-logo.svg" loading="lazy" alt="" class="admin-logo-login mx-1">
      ...
      <a href="establishment-management.php" aria-current="page" class="nav-item w-inline-block w--current"><img src="images/mail-line.svg" loading="lazy" alt="">
        <div class="nav-text">Establishment Management</div>
      </a>
      <a href="contribution-management.php" class="nav-item w-inline-block"><img src="images/mail-line_1.svg" loading="lazy" alt="">
        <div class="nav-text selected">Contribution Management</div>
      </a>
    </div>
    <div class="right-panel">
      ...
      <div class="page-container">
        <h1 class="page-title">Contribution Management</h1>
              ...
              <div class="box table mb-2">
                ...
                <?php
                    if($link){ // READ FUNCTION
                      $userRoleUser = 2;
                        if(isset($_GET['search-user'])) {
                          ...
                          mysqli_select_db($link, "diskobre");
                          $userSearch = mysqli_query($link, "SELECT * from establishment JOIN user ON establishment.user_fk = user.user_id where estab_name LIKE '%{$searchedUser}%' AND user.user_type = '2';");
                            ...
                            while($s = mysqli_fetch_array($userSearch)){
                              ...
                              <div class="w-layout-grid table-grid row estab">
                                ...
                                <div id="w-node-_52b6058c-9090-5212-60aa-9c6e596c737a-48df79eb" class="cell-holder">
                                  <div class="cell-data">Inactive</div>
                                </div>
                                <div id="w-node-_07237bdd-3fc1-a28d-53fb-d1319bdafb41-48df79eb" class="cell-holder">
                                    <a href="view-establishment.php?estab_id=<?php echo $s['establishment_id'];?>&type=user"><img id="view-<?php echo($s['establishment_id']) ?>" loading="lazy" src="images/Group-418.svg" alt="" class="action-icon view-btn"></a>
                   ...
                   <?php  }
                          ...
                          $user = mysqli_query($link, "SELECT * from establishment JOIN user ON establishment.user_fk = user.user_id where user.user_type = '2';");
